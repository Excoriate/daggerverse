...
import (
	...
	"github.com/Excoriate/daggerverse/gotest/examples/go/internal/dagger"
	"github.com/sourcegraph/conc/pool"
)

// Go is a Dagger module that exemplifies the usage of the Gotest module.
//
...
// other specific functionalities of the Gotest module.
...
func (m *Go) BuiltInRecipes(ctx context.Context) error {
	...
	if err := m.Gotest_PassedEnvVars(ctx); err != nil {
		return fmt.Errorf("failed to pass environment variables: %w", err)
	}

	// Run an arbitrary command in the container using Gotest_RunArbitraryCommand
	if _, err := m.Gotest_RunArbitraryCommand(ctx); err != nil {
		return fmt.Errorf("failed to run arbitrary command: %w", err)
	}

	// Create a new container.
	if _, err := m.Gotest_CreateContainer(ctx); err != nil {
