...
func (m *Tests) TestAPIPassingEnvVars(ctx context.Context) error {
	targetModule := dag.
		Brew().
		WithEnvironmentVariable("HOST", "localhost", BrewWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("PORT", "8080", BrewWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("USER", "me", BrewWithEnvironmentVariableOpts{
			Expand: false,
		})
