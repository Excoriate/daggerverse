...
func (m *Tests) TestAPIPassingEnvVars(ctx context.Context) error {
	targetModule := dag.
		Ctrutils().
		WithEnvironmentVariable("HOST", "localhost", CtrutilsWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("PORT", "8080", CtrutilsWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("USER", "me", CtrutilsWithEnvironmentVariableOpts{
			Expand: false,
		})
