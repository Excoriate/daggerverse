...
func (m *Tests) TestGoWithGoServerSimple(ctx context.Context) error {
	// Create the Go server using the Registryauth, specifying the port and source directory.
	goServer := dag.Registryauth(dagger.RegistryauthOpts{
		Ctr: dag.Container().From("golang:1.23-alpine"),
	}).NewGoServer()

	// Configure the Go server.
	goServer = goServer.
		WithSource(m.TestDir, dagger.RegistryauthGoServerWithSourceOpts{
...
func (m *Tests) TestGoWithGoServerAdvanced(ctx context.Context) error {
	...
	goServer = goServer.WithSource(m.TestDir, dagger.RegistryauthGoServerWithSourceOpts{
		Workdir: "golang-server-http",
	})
	...
	goServer = goServer.WithCompileOptions(dagger.RegistryauthGoServerWithCompileOptionsOpts{
		Verbose: true,
	})
