// A generated module for test the Registryauth functions
//
...
func (m *Tests) TestPrintEnvVars(ctx context.Context) error {
	// Retrieve the environment variables using the Registryauth's PrintEnvVars function.
	envVars, err := dag.Registryauth().PrintEnvVars(ctx)
...
func (m *Tests) TestInspectEnvVar(ctx context.Context) error {
	...
	targetModule = targetModule.
		WithEnvironmentVariable(key, value, dagger.RegistryauthWithEnvironmentVariableOpts{
			Expand: true,
		})
