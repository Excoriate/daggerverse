...
func (m *Tests) testTerragruntVersions(ctx context.Context, targetModule *dagger.Registryauth, isAlpine bool) error {
...
func (m *Tests) verifyModule(
	ctx context.Context,
	targetModule *dagger.Registryauth,
	terragruntVersion, tfVersion string,
	isAlpine bool,
) error {
	var opts interface{}
	if isAlpine {
		opts = dagger.RegistryauthWithTerragruntAlpineOpts{
			Version:   terragruntVersion,
			TfVersion: tfVersion,
		}

		alpineOpts, ok := opts.(dagger.RegistryauthWithTerragruntAlpineOpts)
		...
		opts = dagger.RegistryauthWithTerragruntUbuntuOpts{
			Version:   terragruntVersion,
			TfVersion: tfVersion,
		}

		ubuntuOpts, ok := opts.(dagger.RegistryauthWithTerragruntUbuntuOpts)
