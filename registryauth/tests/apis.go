...
func (m *Tests) TestWithEnvironmentVariable(ctx context.Context) error {
	targetModule := dag.
		Registryauth().
		WithEnvironmentVariable("HOST", "localhost", dagger.RegistryauthWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("PORT", "8080", dagger.RegistryauthWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("USER", "me", dagger.RegistryauthWithEnvironmentVariableOpts{
			Expand: false,
		})
...
func (m *Tests) TestWithDownloadedFile(ctx context.Context) error {
	...
	targetModule = targetModule.
		WithDownloadedFile(fileURL, dagger.RegistryauthWithDownloadedFileOpts{
...
func (m *Tests) TestWithClonedGitRepo(ctx context.Context) error {
	...
	targetModule = targetModule.
		WithClonedGitRepo("https://github.com/excoriate/daggerverse",
			dagger.RegistryauthWithClonedGitRepoOpts{})
