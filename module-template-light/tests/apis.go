...
func (m *Tests) TestWithEnvironmentVariable(ctx context.Context) error {
	targetModule := dag.
		ModuleTemplateLight().
		WithEnvironmentVariable("HOST", "localhost", dagger.ModuleTemplateLightWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("PORT", "8080", dagger.ModuleTemplateLightWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("USER", "me", dagger.ModuleTemplateLightWithEnvironmentVariableOpts{
			Expand: false,
		})
...
func (m *Tests) TestWithDownloadedFile(ctx context.Context) error {
	...
	targetModule = targetModule.
		WithDownloadedFile(fileURL, dagger.ModuleTemplateLightWithDownloadedFileOpts{
