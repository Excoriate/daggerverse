...
func (m *Tests) TestGoWithGoServerSimple(ctx context.Context) error {
	...
	goServer := dag.Gotoolbox(dagger.GotoolboxOpts{
		Ctr: dag.Container().From("golang:1.23-alpine"),
	}).NewGoServer()

	// Configure the Go server.
	goServer = goServer.
		WithSource(m.TestDir, dagger.GotoolboxGoServerWithSourceOpts{
...
func (m *Tests) TestGoWithGoServerAdvanced(ctx context.Context) error {
	...
	goServer = goServer.WithSource(m.TestDir, dagger.GotoolboxGoServerWithSourceOpts{
		Workdir: "golang-server-http",
	})
	...
	goServer = goServer.WithCompileOptions(dagger.GotoolboxGoServerWithCompileOptionsOpts{
		Verbose: true,
	})
