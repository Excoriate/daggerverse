...
import (
	...
	"github.com/Excoriate/daggerverse/gotoolbox/examples/go/internal/dagger"
	"github.com/sourcegraph/conc/pool"
)

// Go is a Dagger module that exemplifies the usage of the Gotoolbox module.
//
...
// other specific functionalities of the Gotoolbox module.
...
//
// This method showcases the use of various built-in recipes provided by the Gotoolbox
...
func (m *Go) BuiltInRecipes(ctx context.Context) error {
	// Pass environment variables to the Gotoolbox module using Gotoolbox_PassedEnvVars
	if err := m.Gotoolbox_PassedEnvVars(ctx); err != nil {
		return fmt.Errorf("failed to pass environment variables: %w", err)
	}

	// Create a configured container using Gotoolbox_CreateContainer
	if _, err := m.Gotoolbox_CreateContainer(ctx); err != nil {
		return fmt.Errorf("failed to create container: %w", err)
	}

	// Run an arbitrary command in the container using Gotoolbox_RunArbitraryCommand
	if _, err := m.Gotoolbox_RunArbitraryCommand(ctx); err != nil {
