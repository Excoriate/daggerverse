...
func (m *Tests) testTerragruntVersions(ctx context.Context, targetModule *dagger.Terragrunt, isAlpine bool) error {
...
func (m *Tests) verifyModule(
	ctx context.Context,
	targetModule *dagger.Terragrunt,
	terragruntVersion, tfVersion string,
	isAlpine bool,
) error {
	var opts interface{}
	if isAlpine {
		opts = dagger.TerragruntWithTerragruntAlpineOpts{
			Version:   terragruntVersion,
			TfVersion: tfVersion,
		}

		alpineOpts, ok := opts.(dagger.TerragruntWithTerragruntAlpineOpts)
		...
		opts = dagger.TerragruntWithTerragruntUbuntuOpts{
			Version:   terragruntVersion,
			TfVersion: tfVersion,
		}

		ubuntuOpts, ok := opts.(dagger.TerragruntWithTerragruntUbuntuOpts)
