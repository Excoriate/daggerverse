...
func (m *Tests) TestGoWithGoServerSimple(ctx context.Context) error {
	// Create the Go server using the Terragrunt, specifying the port and source directory.
	goServer := dag.Terragrunt(dagger.TerragruntOpts{
		Ctr: dag.Container().From("golang:1.23-alpine"),
	}).NewGoServer()

	// Configure the Go server.
	goServer = goServer.
		WithSource(m.TestDir, dagger.TerragruntGoServerWithSourceOpts{
...
func (m *Tests) TestGoWithGoServerAdvanced(ctx context.Context) error {
	...
	goServer = goServer.WithSource(m.TestDir, dagger.TerragruntGoServerWithSourceOpts{
		Workdir: "golang-server-http",
	})
	...
	goServer = goServer.WithCompileOptions(dagger.TerragruntGoServerWithCompileOptionsOpts{
		Verbose: true,
	})
