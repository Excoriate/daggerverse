...
import (
	...
	"github.com/excoriate/daggerverse/module-template/tests/dagger/internal/dagger"
...
type ModuleTemplateID = dagger.ModuleTemplateID
...
type ModuleTemplate = dagger.ModuleTemplate

type WithModuleTemplateFunc = dagger.WithModuleTemplateFunc

// ModuleTemplateWithEnvironmentVariableOpts contains options for ModuleTemplate.WithEnvironmentVariable
type ModuleTemplateWithEnvironmentVariableOpts = dagger.ModuleTemplateWithEnvironmentVariableOpts

// ModuleTemplateWithSourceOpts contains options for ModuleTemplate.WithSource
type ModuleTemplateWithSourceOpts = dagger.ModuleTemplateWithSourceOpts
...
type ModuleTemplateOpts = dagger.ModuleTemplateOpts
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Tests":
		...
		case "TestAPIWithContainer":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return nil, (*Tests).TestAPIWithContainer(&parent, ctx)
		case "TestTerminal":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "TestAPIPassingEnvVarsInConstructor":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return nil, (*Tests).TestAPIPassingEnvVarsInConstructor(&parent, ctx)
		case "TestAPIPassingEnvVars":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return nil, (*Tests).TestAPIPassingEnvVars(&parent, ctx)
		case "TestAPIWithSource":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return nil, (*Tests).TestAPIWithSource(&parent, ctx)
		...
		return dag.Module().
			...
			WithObject(
				dag.TypeDef().WithObject("Tests", TypeDefWithObjectOpts{Description: "Tests is a collection of tests.\n\nIt's a struct that contains a single field, TestDir, which is a pointer to a Directory."}).
					...
					WithFunction(
						dag.Function("TestAPIWithContainer",
							dag.TypeDef().WithKind(VoidKind).WithOptional(true)).
							WithDescription("TestAPIWithContainer tests if the container is set correctly.\n\nThis API is used to override the container set in the Dagger instance.")).
					WithFunction(
					...
					WithFunction(
						dag.Function("TestAPIPassingEnvVarsInConstructor",
							dag.TypeDef().WithKind(VoidKind).WithOptional(true)).
							WithDescription("TestAPIPassingEnvVarsInConstructor tests if the environment variables are passed correctly in the constructor.\n\nThis is a helper method for tests, in order to test if the env vars are passed correctly in the constructor.")).
					WithFunction(
						dag.Function("TestAPIPassingEnvVars",
							dag.TypeDef().WithKind(VoidKind).WithOptional(true)).
							WithDescription("TestAPIPassingEnvVars tests if the environment variables are passed correctly in the API.\n\nThis is a helper method for tests, in order to test if the env vars are passed correctly in the API.")).
					WithFunction(
						dag.Function("TestAPIWithSource",
							dag.TypeDef().WithKind(VoidKind).WithOptional(true)).
							WithDescription("TestAPIWithSource tests if the source directory is set correctly.")).
					WithFunction(
