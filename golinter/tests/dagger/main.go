...
func (m *Tests) TestAPIPassingEnvVars(ctx context.Context) error {
	targetModule := dag.
		Golinter().
		WithEnvironmentVariable("HOST", "localhost", GolinterWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("PORT", "8080", GolinterWithEnvironmentVariableOpts{
			Expand: false,
		})

	targetModule = targetModule.
		WithEnvironmentVariable("USER", "me", GolinterWithEnvironmentVariableOpts{
			Expand: false,
		})
