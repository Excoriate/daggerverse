...
import (
	...
	"github.com/Excoriate/daggerverse/tflinter/examples/go/internal/dagger"
	"github.com/sourcegraph/conc/pool"
)

// Go is a Dagger module that exemplifies the usage of the Tflinter module.
//
...
// other specific functionalities of the Tflinter module.
func (m *Go) AllRecipes(ctx context.Context) error {
	...
	// From this point onwards, we're testing the specific functionality of the Tflinter module.
...
func (m *Go) BuiltInRecipes(ctx context.Context) error {
	...
	if err := m.Tflinter_PassedEnvVars(ctx); err != nil {
		return fmt.Errorf("failed to pass environment variables: %w", err)
	}

	// Run an arbitrary command in the container using Tflinter_RunArbitraryCommand
	if _, err := m.Tflinter_RunArbitraryCommand(ctx); err != nil {
		return fmt.Errorf("failed to run arbitrary command: %w", err)
	}

	// Create a new container.
	if _, err := m.Tflinter_CreateContainer(ctx); err != nil {
...
// Tflinter_PassedEnvVars demonstrates how to pass environment variables to the Tflinter module.
//
// This method configures a Tflinter module to use specific environment variables from the host.
func (m *Go) Tflinter_PassedEnvVars(ctx context.Context) error {
	targetModule := dag.Tflinter(dagger.TflinterOpts{
...
// Tflinter_OpenTerminal demonstrates how to open an interactive terminal session
// within a Tflinter module container.
//
// This function showcases the initialization and configuration of a
// Tflinter module container using various options like enabling Cgo,
...
func (m *Go) Tflinter_OpenTerminal() *dagger.Container {
	// Configure the Tflinter module container with necessary options
	targetModule := dag.Tflinter()
...
// Tflinter_RunArbitraryCommand runs an arbitrary shell command in the test container.
//
// This function demonstrates how to execute a shell command within the container
// using the Tflinter module.
//
// Parameters:
//
...
func (m *Go) Tflinter_RunArbitraryCommand(ctx context.Context) (string, error) {
	targetModule := dag.Tflinter().WithSource(m.TestDir)
...
// Tflinter_CreateContainer initializes and returns a configured Dagger container.
//
// This method exemplifies the setup of a container within the Tflinter module using the source directory.
//
// Parameters:
//   - ctx: The context for controlling the function's timeout and cancellation.
//
...
//
// Steps Involved:
//  1. Configure the Tflinter module with the source directory.
//  2. Run a command inside the container to check the OS information.
func (m *Go) Tflinter_CreateContainer(ctx context.Context) (*dagger.Container, error) {
	targetModule := dag.
		Tflinter().
		BaseAlpine().
